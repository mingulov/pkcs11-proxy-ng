...
/// Build-time native-FFI qualifier for v0.2: Linux GNU/musl on x86_64 or
/// aarch64 with 64-bit pointers or x86 with 32-bit pointers, macOS on
/// aarch64 or x86_64
...
pub(in crate::ffi) const NATIVE_FFI_QUALIFIED: bool = (cfg!(target_os = "linux")
    ...
    && ((cfg!(target_arch = "x86_64") && cfg!(target_pointer_width = "64"))
        ...
        || (cfg!(target_arch = "aarch64") && cfg!(target_pointer_width = "64"))))
    || (cfg!(target_os = "macos")
        ...
        match self {
            DomainError::UnsupportedPlatform { detail } => write!(
                ...
                "native FFI unavailable on this platform ({detail}); v0.2 requires \
                 Linux GNU/musl on x86_64 or aarch64 (64-bit) or x86 (32-bit), \
